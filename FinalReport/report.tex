...
SVD-Ridge/Lasso remain competitive linear baselines, and MF-ALS still underperforms mainly on top-10 ranking quality under our current explicit-feedback configuration.
...
\begin{itemize}
\item We provide a controlled comparison of four CF models---MF-SGD, a deep hybrid CNN model, SVD with linear calibration (Ridge/Lasso), and MF-ALS---under identical data conditions.
...
We experiment with two configurations: \textbf{SVD-Ridge} ($k{=}200$, $\alpha{=}0.08$) and \textbf{SVD-Lasso} ($k{=}200$, $\alpha{=}0.008$, coordinate descent with up to 2000 iterations, tolerance $10^{-5}$).
Both use bias regularization $\lambda_b{=}12$.
...
\textbf{Option~3} (SVD + Ridge/Lasso) relies on sparse linear algebra and calibrated linear regressors from a standard machine-learning library; \textbf{Option~4} (MF-ALS) is a NumPy-based alternating least-squares loop with validation-based early stopping.
...
\begin{tabular}{lcccc}
\toprule
 & \textbf{MF-SGD} & \textbf{Deep} & \textbf{SVD-R/L} & \textbf{MF-ALS} \\
\midrule
$k$ (factors) & 64 & 96 & 200 / 200 & 96 \\
Epochs & 150 & 50 & 1 & 40 \\
Batch size & 131k & 65k & -- & -- \\
Learning rate & 0.0050 & 0.0008 & -- & -- \\
LR decay & $0.99\times$ & plateau & -- & -- \\
Reg.\ $\lambda$ / $\alpha$ & 0.05 & $10^{-5}$ $L_2$ & 0.08 / 0.008 & 0.16 \\
Bias reg. & -- & -- & 12.0 & 6.0 \\
Dropout & -- & 0.18 & -- & -- \\
...
\textbf{SVD-Ridge} remains the stronger linear-calibration baseline, \textbf{SVD-Lasso} is less accurate under the current sparse calibration settings, and \textbf{MF-ALS} still trails the top two models.
...
SVD-Ridge          & 0.6651 & 0.8637 \\
SVD-Lasso          & 0.6961 & 0.9008 \\
...
SVD-Lasso   & 0.0282 & 0.0063 & 0.0087 & 0.0324 \\
SVD-Ridge   & 0.0174 & 0.0030 & 0.0044 & 0.0196 \\
...
SVD-Lasso slightly edges SVD-Ridge on NDCG, while MF-ALS remains far behind.
...
This contrasts with our earlier MovieLens-Small study, where stronger regularization on low-rank MF-ALS and SVD hybrids improved NDCG disproportionately.
...
MF-ALS remains the weakest model in top-$N$ ranking on this split, which suggests that this explicit-feedback MF-ALS setup still needs deeper rank/regularization redesign.
...
While we tuned presets documented in the repository, we did not run large-scale automated hyperparameter search for every model; in particular, MF-ALS likely has better operating points than those reported here.
...
Our updated experiments show that (1) the Deep Hybrid model achieves the best overall results (MAE\,=\,0.6005, NDCG@10\,=\,0.0722); (2) tuned MF-SGD becomes a close second and achieves the best RMSE (0.8061 vs.\ 0.8148 for Deep); (3) SVD-Ridge remains a solid low-complexity baseline (MAE\,=\,0.6651), while SVD-Lasso is weaker under the current calibration setting (MAE\,=\,0.6961); and (4) MF-ALS improves in rating error versus earlier presets but remains weak in top-$N$ ranking.

Future work includes dedicated MF-ALS and MF-SGD sweeps at web scale, benchmarking on frozen MovieLens \emph{benchmark} splits for publication-grade comparability, and learning-to-rank objectives that align training with NDCG.
